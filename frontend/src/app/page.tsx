      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 h-16 transition-all duration-300 ${
        ...
        <div className="max-w-[1200px] mx-auto px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer" className="text-[15px] font-medium text-[var(--color-body)] hover:text-[var(--color-ink)] transition-colors">GitHub</a>
            <button
            ...
            {session ? (
              <div className="relative">
                <button onClick={() => setShowUserMenu(!showUserMenu)}
                  className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-lg hover:bg-[var(--color-surface-strong)] transition-colors cursor-pointer"
                  title="Account">
                  {session.user?.image ? (
                    <img src={session.user.image} alt="" className="w-7 h-7 rounded-full object-cover" />
                  ) : (
                    <div className="w-7 h-7 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] text-[11px] font-semibold flex items-center justify-center">
                      {session.user?.name?.charAt(0) || "U"}
                    </div>
                  )}
                  <span className="text-[13px] font-medium text-[var(--color-ink)] hidden lg:block max-w-[100px] truncate">
                ...
                {showUserMenu && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 z-50 w-56 py-2 rounded-xl border border-[var(--color-hairline)] bg-[var(--color-surface-card)] shadow-lg">
                      <div className="px-4 py-2.5 border-b border-[var(--color-hairline)]">
                        <div className="text-[13px] font-medium text-[var(--color-ink)] truncate">{session.user?.name}</div>
                        <div className="text-[11px] text-[var(--color-muted)] truncate mt-0.5">{session.user?.email}</div>
                        <div className="flex items-center gap-1.5 mt-2">
                          {session.user?.provider === "github" && (
                            <svg className="w-3.5 h-3.5 text-[var(--color-muted-soft)]" viewBox="0 0 24 24" fill="currentColor">
                          ...
                          <span className="text-[10px] text-[var(--color-muted-soft)] capitalize">{session.user?.provider}</span>
                        </div>
                      </div>
                      <button onClick={() => { signOut(); setShowUserMenu(false); }}
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-left text-[13px] font-medium text-[var(--color-body)] hover:bg-[var(--color-surface-strong)] transition-colors cursor-pointer">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m-6-6H21m0 0l-3-3m3 3l-3 3" />
                        </svg>
                        Sign out
                      </button>
                    </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[var(--color-canvas)] border-b border-[var(--color-hairline)] px-6 py-6 space-y-4 flex flex-col items-start animate-fade-in">
            ...
            <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer" onClick={() => setIsMobileMenuOpen(false)} className="text-[15px] font-medium text-[var(--color-body)] hover:text-[var(--color-ink)] transition-colors w-full py-1">GitHub</a>
            <button
              onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
            ...
            {session ? (
              <div className="flex flex-col gap-3 w-full pt-2 border-t border-[var(--color-hairline)]">
                <div className="flex items-center gap-2.5 px-1">
                  {session.user?.image ? (
                    <img src={session.user.image} alt="" className="w-8 h-8 rounded-full object-cover" />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] text-[12px] font-semibold flex items-center justify-center">
                      {session.user?.name?.charAt(0) || "U"}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {session.user?.provider === "github" && (
                        <svg className="w-3 h-3 text-[var(--color-muted-soft)] shrink-0" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                        </svg>
                      )}
                      {session.user?.provider === "google" && (
                      ...
                      <span className="text-[11px] text-[var(--color-muted)] truncate">{session.user?.email || session.user?.name}</span>
                    </div>
                  </div>
                </div>
                <button onClick={() => signOut()}
                  className="flex items-center gap-2 text-[13px] font-medium text-[var(--color-body)] hover:text-[var(--color-ink)] transition-colors cursor-pointer px-1">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m-6-6H21m0 0l-3-3m3 3l-3 3" />
                  </svg>
                  Sign out
        ...
        <div className="max-w-[1200px] mx-auto px-6 w-full grid grid-cols-1 md:grid-cols-12 gap-16 items-center relative z-10">
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left md:col-span-5 relative z-10 w-full">
            ...
            <div className="fade-up flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4 w-full sm:w-auto">
              ...
              <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer"
      ...
      <section id="how-it-works" className="relative z-10 py-28 px-6 bg-[var(--color-canvas)] border-t border-[var(--color-hairline)]">
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="pipeline-heading text-center mb-16">
            ...
            <h2 className="text-2xl md:text-3xl font-serif font-medium text-[var(--color-ink)] mt-4 leading-tight">
              ...
              >
                <div className="relative z-10 p-6 flex flex-col gap-4 h-full">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono font-semibold text-[var(--color-muted)] tracking-widest">
                      {step.num} / {step.label}
                    </span>
                    <div className="w-9 h-9 rounded-xl bg-[var(--color-surface-strong)] flex items-center justify-center text-[var(--color-ink)] group-hover:bg-[var(--color-ink)] group-hover:text-[var(--color-canvas)] transition-all duration-300">
                      {step.icon}
                    </div>
                  </div>
                  ...
                  <div className="flex-1">
                    <h3 className="text-base font-serif font-medium text-[var(--color-ink)] mb-1.5 leading-tight">
                      {step.title}
                    </h3>
                  ...
                  {/* Real UI snippet overlays */}
                  {step.num === "01" && (
                    <div className="mt-auto p-3 rounded-xl bg-[var(--color-canvas-soft)] border border-[var(--color-hairline)] font-mono text-[10px] text-[var(--color-muted)] space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--color-ink)] font-semibold">ingest(specs.pdf)</span>
                        <span className="text-[var(--color-ink)] font-semibold tabular-nums">84%</span>
                      </div>
                      <div className="w-full bg-[var(--color-hairline-strong)]/30 h-1 rounded-full overflow-hidden">
                        <div className="bg-[var(--color-ink)] h-full w-[84%] rounded-full transition-all duration-500 group-hover:w-[92%]" />
                      </div>
                      <span className="text-[9px] text-[var(--color-muted-soft)] block">Extracting semantic entities...</span>
                    </div>
                  )}
                  ...
                  {step.num === "02" && (
                    <div className="mt-auto p-3 rounded-xl bg-[var(--color-canvas-soft)] border border-[var(--color-hairline)] font-mono text-[10px] text-[var(--color-muted)] space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--color-ink)] font-semibold">contradiction_check()</span>
                        <span className="text-[var(--color-semantic-error)] font-bold bg-[var(--color-semantic-error)]/10 px-1.5 py-0.5 rounded text-[9px] border border-[var(--color-semantic-error)]/20">1 conflict</span>
                      </div>
                      <span className="text-[9px] text-[var(--color-muted-soft)] block leading-tight">
                  ...
                  {step.num === "04" && (
                    <div className="mt-auto p-3 rounded-xl bg-[var(--color-canvas-soft)] border border-[var(--color-hairline)] font-mono text-[10px] text-[var(--color-muted)] space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--color-ink)] font-semibold">cognee.forget()</span>
                        <span className="text-[var(--color-semantic-error)] font-bold animate-pulse text-[9px]">Pruning</span>
                      </div>
                      <span className="text-[9px] text-[var(--color-muted-soft)] block leading-tight">
      ...
      <footer id="footer" className="relative z-10 bg-[var(--color-canvas)] border-t border-[var(--color-hairline)] py-16 px-6 select-none text-[13px]">
        <div className="max-w-[1200px] mx-auto grid grid-cols-2 md:grid-cols-5 gap-12 text-[var(--color-body)]">
          ...
          <div className="flex flex-col gap-3">
            ...
            <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer" className="hover:text-[var(--color-ink)] transition-colors">GitHub</a>
            <a href="https://github.com/IamNishant51/Synapse/blob/main/README.md" target="_blank" rel="noreferrer" className="hover:text-[var(--color-ink)] transition-colors">Documentation</a>
            <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer" className="hover:text-[var(--color-ink)] transition-colors">Video Demo</a>
          </div>
        ...
        <div className="max-w-[1200px] mx-auto mt-16 pt-8 border-t border-[var(--color-hairline)] flex flex-col md:flex-row items-center justify-between gap-4 text-[var(--color-muted)]">
          ...
          <a href="https://github.com/IamNishant51/Synapse" target="_blank" rel="noreferrer" className="hover:text-[var(--color-ink)] transition-colors">Repository Home</a>
        </div>
      </footer>
    </div>
  );
